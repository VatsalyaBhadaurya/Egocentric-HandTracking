...
try:
    from mpl_toolkits.mplot3d import Axes3D  # noqa — registers 3d projection; not needed in mpl>=3.4
except ImportError:
    pass
        ...
        for si, sname in enumerate(slot_names):
            if self.pinch_log[si]:
                ...
                ax_p.plot(ts, vals, linewidth=0.7,
                          color=["#c090ff", "#50c8ff"][si],
                          label=sname, alpha=0.85)
        pinch_thr = GestureAbstractor.PINCH_THR
        ax_p.axhline(pinch_thr, color="#00C8C8", linestyle="--",
                     linewidth=0.8, alpha=0.6, label=f"pinch thr {pinch_thr:.2f}")
...
class RobotLearningHandPipeline:
    ...
    def __init__(self, device="auto", score_thr=0.12,
        ...
        if self.using_realsense:
            ...
            self.depth_label = "DA2"
            self.calib_done_default = False
        else:
            raise ValueError(f"Unknown camera_backend '{camera_backend}'. "
                             "Choose from: opencv, realsense")
    ...
    def _draw_depth_inset(self, frame, depth):
        # Renders a small colourised depth map inset in the bottom-left corner of the frame.
        ...
        label = f"{self.depth_label} {'CAL' if self.calib_done else 'UNCAL'}"
    ...
    def _draw_calib_ui(self, frame, hands):
        # Overlays a 3-second calibration countdown and progress bar.
        # Returns (palm_kp2d_or_None, done) — palm_kp2d is provided on every frame a
        # hand is visible so samples can be accumulated over the whole window, and
        # done becomes True once the countdown expires.
        ...
        ckp = hands[0]["keypoints"][:,:2] if hands else None
        return ckp, elapsed >= CALIB_SECS
    ...
    def _draw_action_label(self, frame, hand_state, action):
        ...
        y = int(np.clip(hand_state.palm_center_2d[1] - 120, 4, frame.shape[0] - 24))
        txt = f"{action.label}  grip:{action.gripper_command:.2f}"
        cv2.rectangle(frame, (x, y), (x + 168, y + 20), (16, 16, 16), -1)
        ...
        while True:
            ...
            if frame.shape[1] != CAM_W or frame.shape[0] != CAM_H:
                ...
                if sensor_depth is not None:
                    sensor_depth = cv2.resize(sensor_depth, (CAM_W, CAM_H),
            ...
            if self.calib_active:
                ...
                if ckp is not None:
                    self.depth.add_calibration_sample(raw_depth, ckp)
                if done:
                    if self.depth.finalize_calibration():
            ...
            if hands:
                for slot, hand in enumerate(hands):
                    ...
                    hand_states.append(build_hand_state(
                        slot=slot,
                        side=hand.get("hand_side", "unknown"),
                        keypoints=kp,
                        scores=sc,
                        gesture=label,
                        meta=meta,
